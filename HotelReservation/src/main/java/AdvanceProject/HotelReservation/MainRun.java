...
//		    String tableID[]={"#1","#2","#3","#4","#5"};   
			JPanel listT = new JPanel(new GridLayout(2,2,1,15));
			
			final JList listA = new JList(listTableIDFree);
			listA.addListSelectionListener(new ListSelectionListener() {
				...
				public void valueChanged(ListSelectionEvent e) {
					...
					for (int i = 0; i < listTableIDFree.length; i++) {
						if(listA.getSelectedValue() == listTableIDFree[i]) {
							list.add(listTableIDFree[i]);
						}
...
//			String row[] = {"001","Thavath", "09876435","#1 #2"};
...
//	-----------------------------------RETRIEVE CREATE TABLE WITH FROM------------------------------------
	public void retrieveTable() {
		...
		try {
			...
			String[] getBusyTable = getListTableReserved.toArray(new String[getListTableReserved.size()]);
			for (int i = 0; i < listTableFreeID.length; i++) {
				for (int j = 0; j < getBusyTable.length; j++) {
					if(listTableFreeID[i].equals(getBusyTable[j])) {
						for (Tables t : tableList) {
							if(t.getID().equals(getBusyTable[j])) {
								t.setOccupied(true);
							}
						}
					}
...
//	-----------------------------------CLEAR FOOD WITH FROM------------------------------------
	...
	public void createReservation_save() {
		...
		reservationID = Integer.parseInt(txtReservationID.getText());
...
//			booking.setReservedTable(null);
	...
	public void createBookingDetail () {
		...
		try {
			String[] tID;
			tID = getListTableReserved.toArray(new String[getListTableReserved.size()]);
			for (int i = 0; i < getListTableReserved.size(); i++) {
				for (int j = 0; j < tID.length; j++) {
					gTableID += tID[j] +" "; 
				}
			}
			BookingDetail bDetail = new BookingDetail(bookingId, String.valueOf(gTableID));
			session.beginTransaction();
	...
	public void retrieveReservation() {
		...
		try {
			...
			session.beginTransaction();
			Query queryResult = session.createQuery("from Booking");
			Query queryDedail = sessionDetail.createQuery("from BookingDetail");
			Query queryBdetail = sessionDetail.createQuery("from Tables");
			
			
			List<Booking> bookingList  = queryResult.getResultList();
			List<BookingDetail> bDetailList = queryDedail.getResultList();
			List<Tables> tList = queryBdetail.getResultList();
			
			for (Booking b : bookingList) {
				for (BookingDetail bD : bDetailList) {
					for (Tables t : tList) {
						if(bD.getReservedbTableID().equals(t.getID())){
							String[] tabl = { b.getBookingID() +"",b.getCustomerName(), b.getContactNo() , t.getID()};
